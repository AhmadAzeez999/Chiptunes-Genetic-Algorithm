...
def extractMelodyFromAudio(audioPath, sampleRateTemp=22050, noteDuration=1.4):
    # Loading the audio file, librosa.load willl return the audio signal
    # And sample rate
    audio, sampleRate = librosa.load(audioPath)

    # Isolating the melody from the audio signal
    ...
    pitches, magnitudes = librosa.piptrack(y=harmonic, sr=sampleRate, hop_length=hopLength)
...
def playMelody(melody, noteDuration=500):
    ...
    for midiNote in melody:
        ...
        sineWave = generateSineWave(freq, noteDuration)
        song += sineWave
